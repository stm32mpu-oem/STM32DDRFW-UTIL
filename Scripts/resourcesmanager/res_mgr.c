...
static ResMgr_Status_t GetBaseAddrRIF_DMA(ResMgr_data_info_t *rif_info, uint8_t res_num)
{
  ...
  else if (rif_info->res_type == RESMGR_RESOURCE_RIF_HPDMA2)
  {
    ...
    DMA_base = HPDMA3;
  DMA_channel_base = (DMA_Channel_TypeDef *)((unsigned long)DMA_base + offset);
...
ResMgr_Status_t ResMgr_Request(ResMgr_Res_Type_t res_type, uint8_t res_num)
{
  ...
  {
#ifdef RESMGR_PRINT_WARNING
    printf("[WARNING]: %s: Npriv AccessErrror - Priv CPU mode =%d Resource mode =%ld - Res num =%d\n\r",
           __func__, cpu_mode, (unsigned long)ResMgr_GetPrivStatus(resmgr_data, res_num), res_num);
#endif
  ...
  if (ResMgr_GetSecStatus(resmgr_data, res_num) == CPU_SECURE && SecureState == CPU_UNSECURE)
  {
    ...
    printf("[WARNING]: %s: Nsec AccessErrror - Secure CPU mode =%d - Resource mode =%ld - Res num =%d\n\r",
           __func__, SecureState, (unsigned long)ResMgr_GetPrivStatus(resmgr_data, res_num), res_num);
#endif
